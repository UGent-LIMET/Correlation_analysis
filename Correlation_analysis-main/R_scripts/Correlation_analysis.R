...
#plot indiv
png(paste(name_project,'_corr_histograms_Vars_set1.png', sep=""), width=7, height=5, units="in", res=150)
plot(p1, col=rgb(1,0,0,1/4), main= "Histogram per set1 variables")
dev.off()
png(paste(name_project,'_corr_histograms_Vars_set2.png', sep=""), width=7, height=5, units="in", res=150)
...
if(min(test, na.rm = TRUE) < minxaxis | max(test, na.rm = TRUE) > maxaxis){
  ...
  png(paste(name_project,'_corr_histograms_Vars_set1+2_overlay.png', sep=""), width=7, height=5, units="in", res=150)
...
if(VARIABLES_SELECTION == SUBSET_VM){
  ...
  #write vm selection
  #write_dataframe_as_txt_file(variables_selection, paste0("corr_variables_selection.txt"))
  try({write_dataframe_as_txt_file(variableMetadata_selection, paste0("corr_VM_selection.txt"))}) #try if nr rows == 0
}
...
png(paste(name_project, "_dotplot_top", as.character(TOP_RANK), "_corr_Rho_Varset1vsVarset2.png", sep=""), width=7, height=5, units="in", res=150)
plot(dotplot_corr)
dev.off()
...
png(paste(name_project, "_corr_boxplot_Rho_Varset1vsVarset2.png", sep=""), width=7, height=5, units="in", res=150)
