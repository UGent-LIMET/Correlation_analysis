...
########normal correlation computing functions part #############
...
Pvalues_correlation_matrixFDR <- function(scaledmatrix_samples1, scaledmatrix_samples2) {
  ...
  #optional 2nd arg
  if(missing(scaledmatrix_samples2)) {
    # complete: removes missing values
...
#############added new functions for partial correlations#####################
partial_correlation_matrix_samples <- function(scaledmatrix_samples1, scaledmatrix_samples2, confounders) {
  suppressMessages(library(psych))
  
  # optional 2nd arg (same logic as original)
  if (missing(scaledmatrix_samples2)) {
    
    data <- scaledmatrix_samples1
    #data <- data[, colnames(data), drop = FALSE] # does nothing
    data <- na.omit(data)
    
    vars <- setdiff(colnames(data), confounders)
    ...
    vars1 <- setdiff(colnames(scaledmatrix_samples1), confounders)
    vars2 <- setdiff(colnames(scaledmatrix_samples2), confounders)
    ...
    correlation_scaledmatrix <- round(r_full[vars1, vars2, drop = FALSE], 3)
  }
  
  return(correlation_scaledmatrix)
}

Pvalues_partial_correlation_matrixFDR <- function(scaledmatrix_samples1, scaledmatrix_samples2, confounders) {
  suppressMessages(library(psych))
...
# use existing logic (changed only latter parts of computing specific metrics):
partial_correlation_analysis_loop <- function(scaledmatrix_samples1, scaledmatrix_samples2, confounders) {
  ...
  correlation.result <- data.frame(
    CompID1 = rep(rownames(r), times = ncol(r)),
